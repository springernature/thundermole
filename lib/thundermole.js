...
function createProxy (mole, options) {
	...
	proxy.on('proxyReq', augmentProxyRequest.bind(null, mole, options));
...
function formatRequestHeaders (headers) {
	return Object.keys(headers).filter(function (name) {
		return name !== 'cookie';
	}).map(function (name) {
...
function handleUserRequest (mole, options, request, response) {
	if (url.parse(request.url).pathname === options.pingUrl) {
		mole.logger.info('handle ' + options.pingUrl + ' request', request.headers);
...
function proxyUserRequest (mole, request, response, apiResponse) {
	mole.logger.info('Proxying request %s to %s', request.url, apiResponse.target);
	mole.logger.info('Request headers: %s', formatRequestHeaders(request.headers));
...
function augmentProxyRequest (mole, options, proxyRequest, request, response, proxyOptions) {
	var appendedData = JSON.stringify(proxyOptions.append || {});
	...
	proxyRequest.setHeader(options.appendHeader, appendedData);
	mole.logger.info('Appended data: %s', appendedData);
